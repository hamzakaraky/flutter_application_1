...
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
// import 'package:flutter_beacon/flutter_beacon.dart'; // معلّق للآيفون
import 'package:permission_handler/permission_handler.dart';
...
class _RadarScreenState extends State<RadarScreen> with TickerProviderStateMixin {
  ...
  // static const _channel = MethodChannel('com.smartroom/focus'); // معلّق للآيفون
  late final AnimationController _pulseController;
  late final AnimationController _sweepController;
  late final AnimationController _fadeController;
  late final AnimationController _detectedGlowController;

  // StreamSubscription<RangingResult>? _streamRanging; // معلّق للآيفون
  // final _region = Region( // معلّق للآيفون
  //   identifier: 'SmartRoom',
  //   proximityUUID: 'FDA50693-A4E2-4FB1-AFCF-C6EB07647825',
  // );
  ...
  void initState() {
    ...
    // _channel.setMethodCallHandler(_handleNativeCall); // معلّق للآيفون
    _initPermissions();
  }

  @override
  void dispose() {
    // _streamRanging?.cancel(); // معلّق للآيفون
    _pulseController.dispose();
    _sweepController.dispose();
    _fadeController.dispose();
    _detectedGlowController.dispose();
  ...
  Widget _buildHeader() {
    return Padding(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          const Text('Smart Radar', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
          IconButton(
            ...
            onPressed: () {
              showModalBottomSheet(
                ...
                builder: (_) => const Padding(
                  ...
                  child: Text(
                    'This is a demo radar app. Beacon code is disabled for now.',
      ...
      child: Stack(
        ...
        children: [
          ...
          Container(
            ...
            child: Center(child: Text(_isDetected ? 'Detected' : 'Scanning')),
          ),
          AnimatedBuilder(
            animation: _sweepController,
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  gradient: SweepGradient(
                    colors: [_accentColor.withOpacity(0.0), _accentColor.withOpacity(0.4)],
                  ),
                ),
              ),
            ),
  ...
  Widget _buildMetrics() {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          _metricBox('Distance', '${_distance.toStringAsFixed(2)} m'),
          _metricBox('RSSI', '$_rssi dBm'),
          _metricBox('TX Power', '$_txPower dBm'),
        ],
      ),
    );
  }
      ...
      onPressed: () {
        showModalBottomSheet(
          ...
          builder: (_) => const Padding(
            padding: EdgeInsets.all(20),
            child: Text(
              'Setup guide: Beacon code is currently disabled.',
              style: TextStyle(fontSize: 16),
            ),
          ),
        );
